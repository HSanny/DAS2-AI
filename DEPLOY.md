...
| `Can't open lib '/opt/microsoft/msodbcsql18/lib64/libmsodbcsql-18.*.so.*' : file not found` | **That file is not missing.** unixODBC prints "file not found" whenever `dlopen` fails, including when a *dependency* of the driver cannot be resolved — so the file it names is the one thing that is present | Rebuild: `docker compose build --no-cache`. The image now installs `libgssapi-krb5-2` explicitly and refuses to build if the driver does not load. To see which library is really missing: `docker compose run --rm das2-migrate sh -c "ldd /opt/microsoft/msodbcsql18/lib64/libmsodbcsql-*.so* \| grep -i 'not found'"` |
