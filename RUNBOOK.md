...
| `Can't open lib '...libmsodbcsql-18.*.so.*' : file not found` | A *dependency* of the driver is missing, not the file named | `docker compose build --no-cache` — the build now verifies the driver loads |
