...
# ---------- Test-mode bypass ----------
# When SKIP_DB_WRITE=1, exit immediately without touching the database.
# Useful for end-to-end testing of the upstream pipeline (data fetch,
# preprocessing, anomaly detection, CSV outputs) without polluting the
# history table with rows that have a misleading SnapshotAtSGT.
if os.getenv("SKIP_DB_WRITE", "0").strip() == "1":
    logger.info("[SKIP_DB_WRITE=1] Skipping all database writes for this run.")
    logger.info("[SKIP_DB_WRITE=1] CSVs in processed/ and output_csv/ are still on disk for inspection.")
    sys.exit(0)
...
merge_queries = {
    ...
    "data": """
        ...
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (Equipment, [Description], [DateTime], CurrValue)
            VALUES (source.Equipment, source.[Description], source.[DateTime], source.CurrValue);
    """,
    "dateDim": """
