...
from das2.report import charts, theme                  # noqa: E402
...
def _cover(pdf: PdfPages, result, stamp: str) -> None:
    ...
    alertable = list(result.alertable)
    ...
    _stat_tiles(fig, [
        ...
        (str(selection.get("held", 0)), "held back",
         "suppressed or below threshold — page 5"),
    ], top=y - 0.01)

    # The P1 list, in full. If there is one thing a reader takes from this
    # document it is this block, so it sits on the cover rather than behind a
    # page turn.
    y_list = y - 0.22
    p1 = sorted([i for i in incidents if i.priority.value in ("P1", "P2")],
                key=lambda i: -i.severity)[:8]
    fig.text(L, y_list, "Act first", fontsize=theme.SIZE_HEADING,
             fontweight=theme.WEIGHT_BOLD, color=theme.INK, va="center")
    y_list -= 0.035

    if not p1:
        fig.text(L, y_list - 0.02,
                 "Nothing at P1 or P2 this run. The network is quiet; "
                 "the detail pages record what was seen and suppressed.",
                 fontsize=theme.SIZE_BODY, color=theme.INK_SECONDARY,
                 va="center")
    else:
        _table(fig, (L, 0.10, R - L, y_list - 0.10),
               [("", 0.05, "center"), ("class", 0.17, "left"),
                ("region", 0.12, "left"), ("sites", 0.28, "left"),
                ("sensors", 0.08, "right"), ("severity", 0.09, "right"),
                ("what to do", 0.21, "left")],
               [(i.priority.value, i.incident_class.value,
                 _region_of(i), _sites_of(i),
                 len(i.cluster.members), f"{i.severity:.0f}",
                 _short_action(i)) for i in p1],
               row_colors=[theme.PRIORITY_COLOR[i.priority.value] for i in p1])
    _close(pdf, fig)
